...
static void do_takeoff()
{
    ...
    if((int)next_nav_command.p1==0){
		takeoff_pitch_cd = 10*100;
	} else {
	...
	next_WP.lat = home.lat + 10000 * RunwayHeading.x;
	next_WP.lng = home.lng + 10000 * RunwayHeading.y;
...
int32_t Hermite(int32_t xi,int32_t mi,int32_t xf,int32_t mf,uint8_t t){

    int32_t h1 = 2*t^3 - 3*t^2 + 1;
    int32_t h2 = t^3 - 2*t^2 + t;
    int32_t h3 = -2*t^3 + 3*t^2;
    int32_t h4 = t^3 - t^2;
...
// Verify_land is where the work is done because verify is looped while do is only single execution

#ifdef CAPSTONE_CODE	

	VectorHolder CurrentHeading;
	CurrentHeading.x = (float)acos(ahrs.yaw);
	CurrentHeading.y = (float)asin(ahrs.yaw);

	for(unsigned int i; i<=WAYPOINT_COUNT;++i){
		LandingWaypoints[i].id = MAV_CMD_NAV_LAND;
		//LandingWaypoints[i].option = 1;
		LandingWaypoints[i].alt = Hermite(current_loc.alt,0,home.alt,0,i/WAYPOINT_COUNT);
		LandingWaypoints[i].lat = Hermite(current_loc.lat,CurrentHeading.x,home.lat,RunwayHeading.x,i/WAYPOINT_COUNT);
		LandingWaypoints[i].lng = Hermite(current_loc.lng,CurrentHeading.y,home.lng,RunwayHeading.y,i/WAYPOINT_COUNT);
	}

	if (landing_waypoint_index == -1)
		landing_waypoint_index = 0;

	set_next_WP(&LandingWaypoints[0]);
...
static bool verify_land()
{
	...
	int32_t wp_land_distance = get_distance(&current_loc,&LandingWaypoints[landing_waypoint_index]);

	if((wp_land_distance > 0) && (wp_land_distance <= g.waypoint_radius)){
		
		if(landing_waypoint_index == WAYPOINT_COUNT)
			return true;
		
		landing_waypoint_index++;
		set_next_WP(&LandingWaypoints[landing_waypoint_index]);
