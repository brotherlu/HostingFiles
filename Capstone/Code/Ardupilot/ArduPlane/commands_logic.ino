...
if (landing_waypoint_index == -1){
	int32_t current_bearing = ahrs.yaw_sensor/18000*3.14;

	for(unsigned int i=0; i<=WAYPOINT_COUNT;++i){
		
		LandingWaypoints[i].id = MAV_CMD_NAV_LAND;
		LandingWaypoints[i].lat = Hermite(current_loc.lat,acos(current_bearing),home.lat,acos(RunwayHeading),i/WAYPOINT_COUNT);
		LandingWaypoints[i].lng = Hermite(current_loc.lng,asin(current_bearing),home.lng,asin(RunwayHeading),i/WAYPOINT_COUNT);
		LandingWaypoints[i].alt = Hermite(current_loc.alt,0,home.alt,0,i/WAYPOINT_COUNT);
	
	}
	
	set_next_WP(&LandingWaypoints[1]);
...
static bool verify_land()
{
	...
	int32_t landing_wp_distance = get_distance(&current_loc,&next_WP);

	if((landing_wp_distance > 0)&&(landing_wp_distance < g.waypoint_radius)){
		set_next_WP(&LandingWaypoints[landing_waypoint_index++]);
	}
