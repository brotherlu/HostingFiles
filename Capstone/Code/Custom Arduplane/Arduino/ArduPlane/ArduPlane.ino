...
#ifdef CAPSTONE_CODE
	bool Runway_Heading_Set = false;
	int32_t Runway_Heading;
#endif
...
void setup() {
    ...
    // Note: DCM = Direction Cosine Matrix; Quaternions are better
    
    /* For compass method to find heading, easier to use GPS heading since we dont expect it to change regularly
    ahrs.update(); // update the ahrs to have fresh values from the compass
    
    Matrix3f dcmHolder = ahrs.get_dcm_matrix();
    
    Runway_Heading = compass.calculate_heading(&dcmHolder);
    */
...
static void update_GPS(void)
{
    ...
    // set the home runway heading for takeoff
    // Note make sure to invert for landing
    
    if(g_gps->fix && !Runway_Heading_Set){
		Runway_Heading = g_gps->ground_course * 0.01; // in degrees
		
		Runway_Heading_Set = true;
		}
